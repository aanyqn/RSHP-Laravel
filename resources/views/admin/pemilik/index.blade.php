...
<div class="d-flex justify-content-between m-3 mt-0">
    ...
    <form action="{{ route('admin.pemilik.create') }}" method="GET">
        ...
        @foreach ($pemilik as $index => $item)
            <tr>
                ...
                <td>
                    ...
                    <form id="delete-form-{{ $item->idpemilik }}" action="{{ route('admin.pemilik.delete', [$item->idpemilik, $item->user->iduser]) }}" method="POST" style="display: none;">
                        @csrf
                        @method('DELETE')
                    </form>
                </td>
